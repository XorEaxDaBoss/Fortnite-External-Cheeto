...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const bool hovered = ItemHoverable(frame_bb, id);
    bool temp_input_is_active = g.NavActivateId == id;
    ...
    if (temp_input_is_active || temp_input_start)
        return InputScalarAsWidgetReplacement(frame_bb, id, label, data_type, p_data, format);// , p_min, p_max);
    ...
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, grab_bb.GetCenter().y), ImVec2(grab_bb.Max.x - 10, grab_bb.GetCenter().y), ImGuiCol_FrameBg);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 4, GetColorU32(ImGuiCol_FrameBg));
    }
